...
from .importance_attn import DualAttenDecoder
...
class HierarchicalEncoder(BartPretrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        max_utt_num_in_a_batch = (
            max(num_utt_ls) if num_utt_ls else max([x.shape[0] for x in gt_input_ids])
        )
        # print("num_utt_ls:",[x.shape[0] for x in gt_input_ids],"max_seq_len:",max([x.shape[1] for x in gt_input_ids]))
        device = (
            gt_input_ids[0].device
            if isinstance(gt_input_ids, list)
            else gt_input_ids.device
        )
        ...
        if isinstance(gt_input_ids, list):
            for _input_ids, _attention_mask in zip(gt_input_ids, gt_attention_mask):
                ...
                low_encoder_last_hidden_state = self.get_low_encoder_output(
                    _input_ids, _attention_mask, with_grad=True
                ).last_hidden_state

                # [num_utt,hid_dim]
                low_encoder_last_hidden_state = self.extract_from_utt(
                    low_encoder_last_hidden_state
                )

                # pad input_for_high_encoder to [max_utt_num,hid_dim]
                index_ls = list(range(num_utt_in_a_conv)) + [0] * (
                    diff
                )  # 0 is randomly picked since it will be masked
                indices = torch.tensor(index_ls).to(device)
                low_encoder_last_hidden_state = (
                    low_encoder_last_hidden_state.index_select(0, indices)
                )
            ...
            # [bs,num_utt]
            low_encoder_attention_mask_batched = torch.tensor(
                low_encoder_attention_mask_batched, dtype=torch.int32
            ).to(device)
            ...
            low_encoder_output_batched = self.get_low_encoder_output(
                gt_input_ids, gt_attention_mask, with_grad=True
            ).last_hidden_state
            # [bs*num_utt_per_batch,hid_dim] 获取encoder的结果，普通bart的结果
            d_model = low_encoder_output_batched.shape[-1]
            low_encoder_output_batched = self.extract_from_utt(
                low_encoder_output_batched, gt_attention_mask=gt_attention_mask
            )
            index_ls = []  # 这个应该是为了batch做准备，复制batch个
            ...
            low_encoder_output_batched = low_encoder_output_batched.index_select(
                0, index_ls
            )
            low_encoder_output_batched = low_encoder_output_batched.view(
                bs, max_utt_num_in_a_batch, d_model
            )  # 弄出batch
    ...
    def extract_from_utt(self, low_encoder_last_hidden_state, gt_attention_mask=None):
        ...
        if self.config.utt_pooling == "extract":
            ...
            return torch.sum(
                low_encoder_last_hidden_state * gt_attention_mask.unsqueeze(-1), dim=1
            )
...
class BartDecoderWithDualCrossAttention(BartDecoder):
    ...
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.layers = nn.ModuleList(
            [
                BartDecoderLayerWithDualCrossAttention(config)
    ...
    def forward(
        ...
        **kwargs,
        ...
        # expand encoder attention mask
        if (
            low_encoder_last_hidden_state is not None
            and low_encoder_attention_mask is not None
        ):
            # [bsz, seq_len] -> [bsz, 1, tgt_seq_len, src_seq_len]
            kwargs["low_encoder_attention_mask"] = _expand_mask(
                low_encoder_attention_mask, inputs_embeds.dtype, tgt_len=input_shape[-1]
            )

        if (
            high_encoder_last_hidden_state is not None
            and high_encoder_attention_mask is not None
        ):
            # [bsz, seq_len] -> [bsz, 1, tgt_seq_len, src_seq_len]
            kwargs["high_encoder_attention_mask"] = _expand_mask(
        ...
        all_cross_attentions = (
            ()
            if (output_attentions and low_encoder_last_hidden_state is not None)
            else None
        )
        next_decoder_cache = () if use_cache else None
...
class DualAttenEncoder(BartEncoder):
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None):
        super().__init__(config)
    ...
    def forward(
        ...
        **kwargs: dict,
        ...
        sorted_attention_mask = kwargs.get("sorted_attention_mask", None)
        if sorted_input_ids is None or sorted_attention_mask is None:
            raise ValueError(
    ...
    def forward(
        ...
        **kwargs,
        ...
        decoder_outputs = self.decoder(
            ...
            return_dict=return_dict,
            **kwargs,
            # imptance_encoder_attention_mask=kwargs["sorted_attention_mask"],
        )
    ...
    def _expand_inputs_for_generation(  # generate step 2 在初始化beam_scorer之后
        ...
        **model_kwargs,
        ...
        if is_encoder_decoder:
            ...
            if "normal_hidden_state" in encoder_outputs.keys():
                encoder_outputs["normal_hidden_state"] = encoder_outputs[
                    "normal_hidden_state"
                ].index_select(0, expanded_return_idx.to(device))
            if "normal_encoder_attn_mask" in encoder_outputs.keys():
                encoder_outputs["normal_encoder_attn_mask"] = encoder_outputs[
                    "normal_encoder_attn_mask"
                ].index_select(0, expanded_return_idx.to(device))
            model_kwargs["encoder_outputs"] = encoder_outputs
            if "imptance_hidden_state" in encoder_outputs.keys():
                encoder_outputs["imptance_hidden_state"] = encoder_outputs[
                    "imptance_hidden_state"
                ].index_select(0, expanded_return_idx.to(device))
