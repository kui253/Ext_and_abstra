...
def data_preprocessing(datasets, tokenizer, training_args, data_args, model_args):
    ...
    unique_speaker_column = (
        data_args.unique_speaker_column
    )  # maybe we should dynamically add new column?
    ...
    def preprocess_function(examples):
        ...
        if model_args.model_type == "graphtransformer":
            discourse_relations = (
                examples["discourse_relations"]
                if "discourse_relations" in examples.keys()
                else None
            )
            model_inputs = {"gt_input_ids": [], "gt_attention_mask": []}
            ...
            for batch_idx in range(bs):
                ...
                if discourse_relations:
                    discourse_relations_per_instance = discourse_relations[
                        batch_idx
                ...
                if len(utts) > data_args.max_utt_num:
                    ...
                    unique_speakers[batch_idx] = unique_speakers[batch_idx][
                        : data_args.max_utt_num
                    ]
                ...
                model_inputs["gt_attention_mask"].append(
                ...
                if "speaker_adj" in model_args.feature_types:
                    model_inputs["speaker_adj"].append(
                        get_speaker_adj(unique_speakers[batch_idx])
                    )
                if "discourse_adj" in model_args.feature_types:
                    model_inputs["discourse_adj"].append(
                        get_discourse_adj(
                            discourse_relations_per_instance,
                            len(utts),
                            data_args.max_utt_num,
                        )
                    )
                if "cooccurrence_adj" in model_args.feature_types:
                    model_inputs["cooccurrence_adj"].append(
                        get_cooccurrence_adj(keywords_per_instance, len(utts))
                    )
                ...
                inputs_str = ""
                for utt_idx in range(
                    min(data_args.max_utt_num, len(source[batch_idx]))
                ):
...
def get_discourse_adj(discourse_relations, utt_num, max_utt_num):
    ...
    if not discourse_relations:
        return ret
    discourse_relations = [
        [int(x.split(" ")[0]), x.split(" ")[1], int(x.split(" ")[2])]
        for x in discourse_relations
    ]
    discourse_relations = [
        x for x in discourse_relations if x[0] < max_utt_num and x[2] < max_utt_num
    ]
    # ret = [[0]*utt_num for _ in range(utt_num)] # 0 is pad embedding
    for rel in discourse_relations:
        ret[rel[0]][rel[2]] = (
            DISCOURSE_RELATIONS.index(rel[1]) + 1
        )  # +1 for avoid padding index in graphtrans embedding
    return ret
