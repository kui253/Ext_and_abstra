...
class FineTuneCallBack(TrainerCallback):
    def __init__(self, model_args):
        if (
            model_args.config_name.split("/")[-1] == "bart_large"
            or model_args.config_name.split("/")[-1] == "bart_large_cnn"
        ):
...
class ShowModelParamsCallBack(TrainerCallback):
    ...
    def on_train_begin(
        ...
        **kwargs,
        ...
        model = kwargs["model"]
        num_params_unfreezed = sum(
            p.numel() for p in model.parameters() if p.requires_grad
        )
        num_param_freezed = sum(
        ...
        logger.info(
            f"The number of model parameters freezed is : {num_param_freezed/(1000**2)}M"
        )
        logger.info(
            f"The number of model parameters unfreezed is : {num_params_unfreezed/(1000**2)}M"
        )
        logger.info("*" * SCREEN_WIDTH)
